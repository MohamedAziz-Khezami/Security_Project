...
export function SecureTransfer() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const input = inputMode === "text" ? inputText : fileInfo
      ...
      const response = await processFile(
        ...
        algorithm,
        key,
        isRSA && action === "decrypt" ? rsaKeys?.privateKey : undefined,
        applyTo,
        partialParams
      )
      ...
      if (response.success) {
        if (action === "hash") {
          ...
          const blob = new Blob([response.processedContent || ""], { type: "text/plain" })
          ...
          setResult({
            ...
            fileContent: response.processedContent,
          })
        }
      } else {
        setResult({
          success: false,
          message: response.message || "An error occurred during processing",
        })
      }
    } catch (error) {
